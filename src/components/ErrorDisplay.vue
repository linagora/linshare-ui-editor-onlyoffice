<template>
  <div class="error-display" v-if="errorType">
    ...
    <span v-else-if="errorType === 'connect'">
      {{ $t("Something went wrong on our side. Please try again later.") }}
    </span>
    <span v-else-if="errorType === 'document:not-found'">{{ $t("Document not found") }}</span>
    <span v-else-if="errorType === 'user:forbidden'">
      {{ $t("You do not have required permission to edit the document") }}
    </span>
    ...
    <div class="actions">
      ...
      <v-btn v-if="action === 'refresh'" depressed color="primary" @click="refresh()">{{ $t("Refresh") }}</v-btn>
    </div>
  </div>
</template>
